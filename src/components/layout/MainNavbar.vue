...
<template>
  <header class="fixed top-0 inset-x-0 z-[100] bg-[#050814]/80 backdrop-blur-xl">
    <div
      class="max-w-7xl mx-auto flex items-center justify-between px-5 py-3"
    ...
    >
      ...
      <div class="hidden lg:flex items-center justify-between w-full" :class="{ 'flex-row-reverse': isArabic }">
        ...
        <!-- NAV LINKS -->
        <nav class="flex items-center bg-[#131A25]/60 border border-white/10 py-3 px-2 rounded-full shadow-lg" :class="{ 'flex-row-reverse': isArabic }">
          <button
            v-for="item in navItems"
            :key="item.id"
            @click="scrollToSection(item)"
            class="px-3 py-2 rounded-full text-[15px] font-medium transition whitespace-nowrap mx-3"
            :class="
              activeId === item.id
        ...
        <div class="flex items-center gap-3" :class="{ 'flex-row-reverse': isArabic }">
          ...
          <button
            @click="goToSignin"
            class="h-[45px] px-8 rounded-full border border-white/30 text-white/90 hover:text-white transition"
          >
          ...
          <button
            @click="goToSignup"
            class="h-[45px] px-8 rounded-full bg-gradient-to-r from-[#2D7DFF] to-[#1B5CFF] text-white font-semibold hover:opacity-90 transition"
          >
            {{ t('landing.nav.startFree') }}
          </button>
        </div>
  ...
  <Teleport to="body">
    ...
    >
      ...
      <div
        class="absolute top-0 bottom-0 w-[85%] max-w-sm overflow-y-auto"
        :class="[
